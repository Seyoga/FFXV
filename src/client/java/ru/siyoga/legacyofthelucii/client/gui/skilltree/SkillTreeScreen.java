...
public final class SkillTreeScreen extends Screen {
    private static final Identifier NOCTIS_BACKGROUND = new Identifier(
            LegacyOfTheLucii.MOD_ID,
            "textures/gui/skill_tree/noctis.png"
    );
    private static final Identifier ARDYN_BACKGROUND = new Identifier(
            LegacyOfTheLucii.MOD_ID,
            "textures/gui/skill_tree/ardyn.png"
    );
    private static final Identifier WINDOW_TEXTURE = new Identifier(
            LegacyOfTheLucii.MOD_ID,
            "textures/gui/skill_tree/window.png"
    );
    private static final Identifier TASK_FRAME_OBTAINED = new Identifier(
            LegacyOfTheLucii.MOD_ID,
            "textures/gui/skill_tree/task_frame_obtained.png"
    );
    ...
    private void renderSkills(DrawContext context, int x, int y, int mouseX, int mouseY) {
        ...
        if (ClientLuciiState.legacy() != LuciiLegacy.NOCTIS) {
            return;
        }
        renderSkillNode(context, x + PAGE_WIDTH / 2 - 55 + (int) Math.round(scrollX), y + PAGE_HEIGHT / 2 - SKILL_FRAME_SIZE / 2 + (int) Math.round(scrollY), Items.IRON_BLOCK.getDefaultStack(), Text.translatable("skill.legacyofthelucii.noctis.wall"), mouseX, mouseY);
        renderSkillNode(context, x + PAGE_WIDTH / 2 - 13 + (int) Math.round(scrollX), y + PAGE_HEIGHT / 2 - SKILL_FRAME_SIZE / 2 + (int) Math.round(scrollY), Items.ENDER_PEARL.getDefaultStack(), Text.translatable("skill.legacyofthelucii.noctis.warp"), mouseX, mouseY);
    ...
    private static Identifier backgroundFor(LuciiLegacy legacy) {
        return legacy == LuciiLegacy.ARDYN ? ARDYN_BACKGROUND : NOCTIS_BACKGROUND;
    }
